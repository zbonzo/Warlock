...
interface AbilityButtonText {
  ready: string;
  submitted: string;
}

interface AbilityButtonTextMap {
  [key: string]: AbilityButtonText;
}
 ...
 */
const ABILITY_BUTTON_TEXT: AbilityButtonTextMap = {
...
export const getActionButtonText = (
  ...
  return isSubmitted ? abilityText?.submitted || 'Action Locked' : abilityText?.ready || 'Submit Action';
};

/**
 * Get the button variant based on ability type and state
 */
export const getActionButtonVariant = (abilityType: string, isSubmitted: boolean): ButtonVariant => {
  if (isSubmitted) {

...
const PlayerCard = ({ player, isCurrentPlayer, canSeeWarlock }) => {
  ...
  const displayName = shouldShowAsWarlock ? toZalgo(characterString) : characterString;

  return (
    <div className={`player-card ${isCurrentPlayer ? 'current-player' : ''} ${!player.isAlive ? 'dead' : ''} ${shouldShowAsWarlock ? 'warlock-player' : ''}`}>
      
      {/* Character name and class at the top */}
      <div className="character-header">
        <h3 className={`character-name ${shouldShowAsWarlock ? 'warlock-text' : ''}`}>
          {displayName}
          {isCurrentPlayer && ' (You)'}
        </h3>
      </div>

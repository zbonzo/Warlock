...
interface GamePageProps {
  ...
  monster: Monster;
  eventsLog: any[];
  onSubmitAction: (actionType: string, selectedTarget: string) => void;
}
...
const GamePage: React.FC<GamePageProps> = ({
  ...
  const lastValidActionRef = useRef<LastValidAction | null>(null);
  ...
  useEffect(() => {
    ...
    const handlePlayerDisconnected = (data: { playerName: string; message: string }) => {
  ...
  const getCharacterTitle = (): string => {
  ...
  useEffect(() => {
    ...
    const handleGameStateUpdate = (data: { phase?: GamePhase }) => {
  ...
  useEffect(() => {
    ...
    const handleAdaptabilityChoose = (data: { abilities?: any }) => {
      console.log(
   ...
   */
  const handleRacialAbilityUse = (abilityType: string) => {
   ...
   */
  const handleReplaceAbility = (oldAbilityType: string, newAbilityType: string, level: number) => {
    if (!socket) {
   ...
   */
  const handleWizardAbilitySelect = (ability: Ability) => {

...
describe('StatusEffectManager', () => {
  ...
  beforeEach(() => {
    ...
    statusEffectManager = new StatusEffectManager(mockPlayers, mockGameStateUtils);
  ...
  describe('constructor', () => {
    ...
    it('should have effect definitions for known effects', () => {
      ...
      expect(statusEffectManager.effectDefinitions).toHaveProperty('protected');
  ...
  describe('applyEffect', () => {
    it('should apply an effect to a player', () => {
      ...
      // Apply effect
      const result = statusEffectManager.applyEffect('player1', 'poison', { damage: 10, turns: 2 }, mockLog);
      
      // Check result
      expect(result).toBe(true);
      expect(mockPlayer.applyStatusEffect).toHaveBeenCalledWith('poison', { damage: 10, turns: 2 });
      expect(mockLog).toContain('Alice is poisoned for 10 damage over 2 turns.');
    });
    ...
    it('should apply effect with default values when parameters missing', () => {
      ...
      const result = statusEffectManager.applyEffect('player1', 'poison', { damage: 10 }, mockLog);
      
      // Check result - should use default turns value
      expect(result).toBe(true);
      expect(mockPlayer.applyStatusEffect).toHaveBeenCalledWith('poison', { damage: 10, turns: 3 });
    });
    
    it('should return false if player does not exist', () => {
      // Apply effect to non-existent player
      const result = statusEffectManager.applyEffect('nonexistent', 'poison', { damage: 10 }, mockLog);
    ...
    it('should log a refresh message if player already has the effect', () => {
      ...
      expect(result).toBe(true);
      expect(mockPlayer.applyStatusEffect).toHaveBeenCalledWith('poison', { damage: 10, turns: 2 });
      expect(mockLog).toContain("Alice's poison is refreshed for 10 damage over 2 turns.");
    });
    
    it('should return false for unknown effect type', () => {
      // Apply unknown effect
      const result = statusEffectManager.applyEffect('player1', 'unknown', { param: 'value' }, mockLog);
      
      // Check result
      expect(result).toBe(false);
      expect(mockPlayer.applyStatusEffect).not.toHaveBeenCalled();
      expect(mockLog).toContain('Unknown effect unknown could not be applied to Alice.');
    });
  });
  ...
  describe('removeEffect', () => {
    it('should remove an effect from a player', () => {
      ...
      // Remove effect
      const result = statusEffectManager.removeEffect('player1', 'poison', mockLog);
    ...
    it('should return false if player does not exist', () => {
      ...
      const result = statusEffectManager.removeEffect('nonexistent', 'poison', mockLog);
  ...
  describe('isPlayerStunned', () => {
    it('should return true if player is stunned', () => {
      ...
      // Check result
      expect(result).toBe(true);
      expect(statusEffectManager.hasEffect).toHaveBeenCalledWith('player1', 'stunned');
    });
  ...
  describe('processTimedEffects', () => {
    it('should process all effects for all alive players', () => {
      ...
      expect(statusEffectManager.processPoisonEffect).toHaveBeenCalledWith(mockPlayer, mockLog);
      expect(statusEffectManager.processTimedEffect).toHaveBeenCalledWith(mockPlayer, 'protected', mockLog);
      expect(statusEffectManager.processTimedEffect).toHaveBeenCalledWith(mockPlayer, 'invisible', mockLog);
      expect(statusEffectManager.processTimedEffect).toHaveBeenCalledWith(mockPlayer, 'stunned', mockLog);
    });
  });
  ...
  describe('processPoisonEffect', () => {
    it('should apply poison damage and decrement turns', () => {
      ...
      mockPlayer.hasStatusEffect.mockImplementation((effect) => effect === 'poison');
  ...
  describe('processTimedEffect', () => {
    it('should decrement turns for a timed effect', () => {
      ...
      mockPlayer.hasStatusEffect.mockImplementation((effect) => effect === 'invisible');
    ...
    it('should remove effect when turns reach 0', () => {
      ...
      mockPlayer.hasStatusEffect.mockImplementation((effect) => effect === 'stunned');
  ...
  describe('message generation', () => {
    it('should generate appropriate application messages for each effect type', () => {
      ...
      expect(statusEffectManager.getEffectApplicationMessage('Alice', 'poison', { damage: 10, turns: 2 }))
        .toBe('Alice is poisoned for 10 damage over 2 turns.');
      
      // Check protected message
      expect(statusEffectManager.getEffectApplicationMessage('Alice', 'protected', { armor: 5, turns: 2 }))
        .toBe('Alice is protected with 5 armor for 2 turn(s).');
      
      // Check invisible message
      expect(statusEffectManager.getEffectApplicationMessage('Alice', 'invisible', { turns: 1 }))
        .toBe('Alice becomes invisible for 1 turn(s).');
      
      // Check stunned message
      expect(statusEffectManager.getEffectApplicationMessage('Alice', 'stunned', { turns: 2 }))
        .toBe('Alice is stunned for 2 turn(s).');
      
      // Check unknown effect
      expect(statusEffectManager.getEffectApplicationMessage('Alice', 'unknown', {}))
        .toBe('Alice is affected by unknown.');
    });
    
    it('should generate appropriate refresh messages for each effect type', () => {
      // Check poison message
      expect(statusEffectManager.getEffectRefreshMessage('Alice', 'poison', { damage: 10, turns: 2 }))
        .toBe("Alice's poison is refreshed for 10 damage over 2 turns.");
      
      // Check protected message
      expect(statusEffectManager.getEffectRefreshMessage('Alice', 'protected', { armor: 5, turns: 2 }))
        .toBe("Alice's protection is refreshed for 2 turn(s).");
      
      // Check invisible message
      expect(statusEffectManager.getEffectRefreshMessage('Alice', 'invisible', { turns: 1 }))
        .toBe("Alice's invisibility is extended for 1 turn(s).");
      
      // Check stunned message
      expect(statusEffectManager.getEffectRefreshMessage('Alice', 'stunned', { turns: 2 }))
        .toBe("Alice remains stunned for 2 more turn(s).");
      
      // Check unknown effect
      expect(statusEffectManager.getEffectRefreshMessage('Alice', 'unknown', {}))
        .toBe("Alice's unknown effect is refreshed.");
    });
    
    it('should generate appropriate expiration messages for each effect type', () => {
      // Check poison message
      expect(statusEffectManager.getEffectExpirationMessage('Alice', 'poison'))
        .toBe('The poison affecting Alice has worn off.');
      
      // Check protected message
      expect(statusEffectManager.getEffectExpirationMessage('Alice', 'protected'))
        .toBe('Alice is no longer protected.');
      
      // Check invisible message
      expect(statusEffectManager.getEffectExpirationMessage('Alice', 'invisible'))
        .toBe('Alice is no longer invisible.');
      
      // Check stunned message
      expect(statusEffectManager.getEffectExpirationMessage('Alice', 'stunned'))
        .toBe('Alice is no longer stunned.');
      
      // Check unknown effect
      expect(statusEffectManager.getEffectExpirationMessage('Alice', 'unknown'))

...
describe('CombatSystem', () => {
  ...
  beforeEach(() => {
    ...
    mockTarget = {
      ...
      isWarlock: false,
      racialEffects: {},
      takeDamage: jest.fn((damage, source) => {
        const reducedDamage = mockTarget.calculateDamageReduction
          ? mockTarget.calculateDamageReduction(damage)
          : damage;
        mockTarget.hp = Math.max(0, mockTarget.hp - reducedDamage);
        if (mockTarget.hp <= 0) {
  ...
  describe('validateAndQueueAction', () => {
    ...
    it('should redirect target if invisible', () => {
      ...
      mockTarget.hasStatusEffect = jest
        .fn()
        .mockImplementation((effect) => effect === 'invisible');
  ...
  describe('applyDamageToPlayer', () => {
    it('should apply damage and reduce HP', () => {
      ...
      expect(mockLog).toContain(
        'Bob takes 30 damage from Charlie. Armor reduced initial 40 damage.'
      );
    });
    ...
    it('should handle Stone Resolve immunity', () => {
      ...
      expect(mockLog).toContain(
        "Bob's Stone Resolve absorbed all damage from Charlie!"
      );
    });
    ...
    it('should trigger Keen Senses to reveal warlock status', () => {
      ...
      expect(result).toBe(true);
      expect(mockLog).toContain(
        "Charlie's Keen Senses reveal that Bob is NOT a Warlock!"
      );
    });
    ...
    it('should use Undying to prevent immediate death', () => {
      ...
      expect(mockLog).toContain(
        'Bob avoided death through Undying! Resurrected with 5 HP.'
      );
    });
  ...
  describe('processPendingDeaths', () => {
    it('should process all pending deaths', () => {
      ...
      expect(mockLog).toContain(
        'Alice has died from wounds inflicted by Monster!'
      );
      expect(mockLog).toContain(
        'Bob has died from wounds inflicted by Charlie!'
      );
    ...
    it('should handle Undying during death processing', () => {
      ...
      expect(mockLog).toContain(
        'Alice avoided death through Undying! Resurrected with 5 HP.'
      );
      expect(mockPlayer.pendingDeath).toBeUndefined();
  ...
  describe('applyAreaDamage', () => {
    it('should apply damage to multiple targets', () => {
      ...
      jest
        .spyOn(combatSystem, 'applyDamageToPlayer')
      ...
      // Execute
      const result = combatSystem.applyAreaDamage(
        mockPlayer,
        30,
        targets,
        mockLog
  ...
  describe('applyAreaHealing', () => {
    it('should heal multiple targets', () => {
      ...
      // Execute
      const result = combatSystem.applyAreaHealing(
        mockPlayer,
        20,
        targets,
        mockLog
      ...
      expect(mockLog).toContain(
        "Alice is healed for 20 HP by Alice's ability."
      );
    ...
    it('should include warlocks when specified', () => {
      ...
      expect(mockLog).toContain("Bob is healed for 20 HP by Alice's ability.");
      expect(mockLog).toContain(
        "Charlie is healed for 20 HP by Alice's ability."
      );
    });

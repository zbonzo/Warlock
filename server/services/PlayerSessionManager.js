...
class PlayerSessionManager {
  constructor() {
    ...
    // Reconnection window in milliseconds (60 seconds)
    this.reconnectionWindow = 60 * 1000;
  ...
  registerSession(gameCode, playerName, socketId, playerData = {}) {
    ...
    logger.debug(`Registered session for ${playerName} in game ${gameCode} with socket ${socketId}`);
  }
  ...
  updateSocketId(gameCode, playerName, newSocketId) {
    ...
    if (gameSession.has(playerName)) {
      ...
      logger.info(`Player ${playerName} reconnected to game ${gameCode} (${oldSocketId} -> ${newSocketId})`);

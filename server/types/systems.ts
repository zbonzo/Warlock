...
export interface GameSystem<TState, TEvent> extends BaseSystem {
   ...
   */
  process(_state: TState, _event: TEvent): Promise<TState>;
   ...
   */
  validate(_event: TEvent): ValidationResult<TEvent>;
   ...
   */
  canHandle(_event: TEvent): boolean;
   ...
   */
  updateConfig(_config: Partial<SystemConfig>): void;
}
...
export interface CombatSystemInterface extends EventDrivenSystem<GameRoom> {
   ...
   */
  calculateDamage(
    _attacker: Player | Monster,
    _defender: Player | Monster,
    _baseDamage: number
  ): DamageCalculation;
   ...
   */
  canAttack(_attacker: Player | Monster, _target: Player | Monster): boolean;
}
...
export interface EffectSystemInterface extends EventDrivenSystem<GameRoom> {
   ...
   * @returns Updated state
   */
  processEffects(_state: GameRoom): Promise<GameRoom>;
   ...
   */
  isImmune(_target: Player | Monster, _effectType: string): boolean;
}
...
export interface PhaseSystemInterface extends EventDrivenSystem<GameRoom> {
   ...
   */
  nextPhase(_state: GameRoom): Promise<GameRoom>;
   ...
   */
  canTransition(_fromPhase: string, _toPhase: string): boolean;
   ...
   */
  getPhaseDuration(_phase: string): number;
   ...
   * @returns Updated state
   */
  handlePhaseTimeout(_state: GameRoom): Promise<GameRoom>;
}
...
export interface SystemManager {
   ...
   */
  registerSystem(_system: BaseSystem): void;

  /**
   * Unregister a system
   * @param systemName - Name of system to unregister
   */
  unregisterSystem(_systemName: string): void;
   ...
   */
  getSystem<T extends BaseSystem>(_systemName: string): T | undefined;
   ...
   * @returns Updated state
   */
  processEvent(_state: GameRoom, _event: GameEvent): Promise<GameRoom>;
}
...
export abstract class AbstractGameSystem<TState, TEvent> implements GameSystem<TState, TEvent> {
  ...
  abstract process(_state: TState, _event: TEvent): Promise<TState>;
  abstract validate(_event: TEvent): ValidationResult<TEvent>;
  abstract canHandle(_event: TEvent): boolean;
}

...
function calculateConversionChance(warlockCount, totalPlayers, modifier = 1.0) {
  ...
  const rawChance = Math.min(maxChance, baseChance + (warlockCount / totalPlayers) * scalingFactor);
...
function calculateDamageReduction(armor) {
  ...
  return Math.min(player.armor.maxReduction, armor * player.armor.reductionRate);
}

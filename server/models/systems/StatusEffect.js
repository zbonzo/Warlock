...
class StatusEffect {
   ...
   * @param {string} type - Effect type (poison, shielded, etc.)
   * @param {Object} params - Effect parameters
   * @param {string} sourceId - ID of who applied this effect
   * @param {string} sourceName - Name of who applied this effect
   * @param {string} targetId - ID of who this effect is on
   */
  constructor(type, params = {}, sourceId = null, sourceName = null, targetId = null) {
   ...
   * @param {Object} target - The target entity (player or monster)
   * @param {Array} log - Event log to append messages to
   * @returns {Object} Processing result
   */
  processTurn(target, log = []) {
   ...
   * @returns {Array} Effects caused
   * @private
   */
  processPoison(target, log) {
    ...
    if (target.hp <= 0) {
      ...
      target.deathAttacker = this.sourceName || 'Poison';
  ...
  processBleed(target, log) {
    ...
    if (target.hp <= 0) {
      target.isAlive = false;
      target.pendingDeath = true;
      target.deathAttacker = this.sourceName || 'Bleed';
   ...
   * @private
   */
  processHealingOverTime(target, log) {
  ...
  onExpired(target, log) {
    ...
    if (this.type === 'stunned') {
      ...
      if (target.isVulnerable !== undefined) {
        ...
        target.vulnerabilityIncrease = 0;
      }
    }
   ...
   * @param {boolean} stackStrength - Whether to add to existing strength
   */
  refresh(newParams = {}, stackStrength = false) {
    // Refresh duration
    if (newParams.turns || newParams.duration) {
      this.turnsRemaining = Math.max(this.turnsRemaining, newParams.turns || newParams.duration);
    }
   ...
   * @param {string} calculationType - Type of calculation (damageDealt, damageTaken, etc.)
   * @returns {Object} Contribution object
   */
  getCalculationContribution(calculationType) {
    const contribution = {
    ...
    switch (this.type) {
      ...
      case 'weakened':
        if (calculationType === 'damageDealt') {
          contribution.percentage = -(this.params.damageReduction * 100) || -25;
        }
        break;

      case 'enraged':
        if (calculationType === 'damageDealt') {
          contribution.percentage = (this.params.damageBoost - 1) * 100 || 50;
        } else if (calculationType === 'damageTaken') {
          contribution.percentage = -(this.params.damageResistance * 100) || -30;
        }
        break;
   ...
   * @returns {boolean} Whether this effect prevents the action
   */
  preventsAction(actionType) {
   ...
   * @param {string} racialType - Type of racial ability
   * @param {string} targetId - Target of the effect
   * @param {Object} params - Racial ability parameters
   * @returns {StatusEffect} New status effect instance
   */
  static createRacialEffect(racialType, targetId, params = {}) {
    const racialParams = {
      ...params,
      isPassive: true,
      isPermanent: true,
